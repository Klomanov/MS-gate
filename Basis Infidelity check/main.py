...
t_gate = np.array([t * 1e-6 for t in range(50, 140)])
...
for n in tqdm(range(len(n_df))):
    for t in range(len(t_gate)):
        for m in range(len(mu)):
            ...
            handler = MSHandler.get_from_chain_with_pulse_shaping(chain, carrier_opt=False,
                                                                  **shaping_attrs)
...
for n in range(len(n_df)):
    ...
    im = axes[row, col].pcolormesh(MU_grid, T_grid * 1e6, np.log(inf[n, :, :]),
                                   shading='auto', cmap='viridis', vmin=-5, vmax=0)
    axes[row, col].set_xlabel('mu (Hz)')
    axes[row, col].set_ylabel('t_gate (µs)')
    axes[row, col].set_title(f'log infidelity for n_df = {n_df[n]}')
    plt.colorbar(im, ax=axes[row, col], label='log infidelity')
