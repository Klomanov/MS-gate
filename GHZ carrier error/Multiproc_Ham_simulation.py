...
if __name__ == '__main__':
    ...
    ideal_op = ideal_MS(N_ions, -np.pi / 4)
    ...
    for i in range(len(t_gate_arr)):
        params = (gamma_arrs[0, i], gamma_arrs[1, i], beta_arrs[0, i], beta_arrs[1, i])
        args = (i, t_gate_arr[i], params, constants)
        tasks.append(args)

    print(f"Запуск {2*len(tasks)} симуляций на {cpu_count()} ядрах...")
    with Pool(cpu_count()) as p:
        results = list(tqdm(p.imap(run_single_simulation_1, tasks), total=len(tasks)))

    results.sort(key=lambda r: r['index'])
    simulated_infidelity_arr_1 = np.array([r['infidelity'] for r in results])
    ...
    plt.plot(t_gate_arr * 1e6, np.log10(inf_arr_one_tone), color="black", linestyle="--", label='One Tone Analytical Infidelity')
    plt.plot(t_gate_arr * 1e6, np.log10(simulated_infidelity_arr_1), color="green", linestyle="--", label='One Tone Simulation Infidelity')
    plt.plot(t_gate_arr * 1e6, np.log10(inf_arr_analytical), 'o-', label='Two Tone Analytical Infidelity', markersize=4)
    plt.plot(t_gate_arr * 1e6, np.log10(simulated_infidelity_arr_2), 's-', label='Two Tone Simulation Infidelity', markersize=4)
    
    plt.title(f'Optimized and simulated infidelity comparsion for {N_ions} ions and {N_modes_to_include} modes')
    plt.xlabel(r'$t_{gate}$, мкс')
